import random # for the random choice of cards
...
deck = {"Ace of Diamonds": 11, "Two of Diamonds" : 2, "Three of Diamonds": 3 , "Four of Diamonds": 4 , "Five of Diamonds" : 5,
        ...
        } # Creating the deck of cards through a dictionary to keep the two parts of a card together

#Creates a function to decide how to account for an Ace an 11 or 1
# with the sum of cards drawn
def score_hand(hand):
...
# Creating a class to handle the player names and
# the hand for cards to be drawn at random
class Players:
...
#starting the program to have user choose to play or exit, if play it states the rules
...
if Start_or_Stop == '' \
    ...
    print("Player Total: ", sum_of_player) # print the sum of cards tat are drawn
    #print("Cards in Hand: ", len(player.hand))#Test
    if sum_of_player == 21: # if 21 with the first 2 cards player wins and game is over
        print("Blackjack, YOU WIN")
        exit()
    choice = input("Do you want to 'HIT' or 'STAY', type your result. ") # allowing user to choose path and play
    decision = choice.upper() # account for user error
    while decision == 'HIT':
        #print(player.hand)#Test
        player.draw()
        #print(player.hand)#Test
        sum_of_player = score_hand(player.hand)
        if sum_of_player == None:
            ...
            exit()
            # if the function returns none that means the all options in the hand with an Ace
            #are above 21 and therefor the player busts.
        print("Player Total: ", sum_of_player)
        #print("Cards in Hand: ", player.hand)#Test
        if sum_of_player == 21:
            print("Blackjack, YOU WIN")
            break
        if len(player.hand) >= 5 and sum_of_player < 21:
            ...
            exit()
            # Base on the rules with 5 cards summing to less than 21 the player wins
        ...
        #accounting for the dealer's turn
    while decision == "STAY":
            dealer.draw()
            sum_of_dealer = score_hand(dealer.hand)
            print("Dealer Total: ", sum_of_dealer)
            #print("Cards in Hand: ", dealer.hand)#Test
            #print(sum_of_dealer)#Test
            if sum_of_player == sum_of_dealer:
                print("push....you Tie, maybe next time.")
                exit()
                #accounting for a tie imediatly after draw
            while sum_of_dealer < 17:# accounting for house rules of 17 to continue to draw
                dealer.draw()
                sum_of_dealer = score_hand(dealer.hand)
                print("Dealer Total: ", sum_of_dealer)
                #print("number of cards in dealer hand", dealer.hand)#Test
                #print("Total of dealer cards", sum_of_dealer)#Test
                if sum_of_player == sum_of_dealer: # accounting for tie after cards drawn
                    print("push....you Tie, maybe next time.")
                    exit()
                if sum_of_dealer == None: # accounting for the function returning none meaning
                    #all possible values are over 21 so the player wins
                ...
                if sum_of_dealer > 21:
                         ...
                         exit()
            if sum_of_dealer >= 17: # not allowing the dealer to draw after having 17 or more
                if sum_of_player > 21:
...
if Start_or_Stop == "NO":
        ...
        exit()
#connected to the top when choosing to start or stop the program
